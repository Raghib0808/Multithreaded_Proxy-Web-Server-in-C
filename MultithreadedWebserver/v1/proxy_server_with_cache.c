...
#include <unistd.h>
#include <fcntl.h>
#include <time.h> //time based lru cache
#include <sys/wait.h>
#include <errno.h>
...
int main(int argc, char *argv[])
{
    ...
    proxy_socketId = socket(AF_INET, SOCK_STREAM, 0); // SOCK_Stream  implies TCP
    ...
    bzero((char *)&server_addr, sizeof(server_addr)); // removes the garbage values from the variables passed to it, needs size of to know till which memory location it needs to do that
    server_addr.sin_family = AF_INET;                 // ipv4
    server_addr.sin_port = htons(port_number);        // Assigning port to the Proxy, and making bigendian and littleendian changes in numbers
